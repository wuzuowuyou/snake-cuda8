...
def load_model(net, optim, scheduler, recorder, model_dir, resume=True, epoch=-1):
    ...
    #################################################################change by yhl 20201012
    state_dict_ = pretrained_model['net']
    state_dict = {}
    # convert data_parallal to model
    for k in state_dict_:
        if k.startswith('module.net.'):
            state_dict[k[11:]] = state_dict_[k]
        else:
            state_dict[k] = state_dict_[k]


    if 'net' in pretrained_model:
        net.load_state_dict(state_dict)  #net.load_state_dict(pretrained_model['net'])
    if 'optim' in pretrained_model:
        optim.load_state_dict(pretrained_model['optim'])
    if 'scheduler' in pretrained_model:
        scheduler.load_state_dict(pretrained_model['scheduler'])
    if 'recorder' in pretrained_model:
